...
function normalizeWishlistItems(items: ServerWishlistItem[]): WishlistItem[] {
  return items
    .map((item) => ({
      ...
      variantMode: item.variantMode === 'none' || item.variantMode === 'size' || item.variantMode === 'color' || item.variantMode === 'both' ? item.variantMode : undefined,
    ...
    }),
    {
      ...
      onRehydrateStorage: () => (state) => {
        if (state) {
          ...
          // Use Zustand's setter so subscribers are notified that persisted state
          // is ready instead of mutating the callback state object silently.
