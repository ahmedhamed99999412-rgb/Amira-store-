...
export function ProductDetailClient({
  ...
  // Derive the required option mode from the actual variant rows, not from
  // the legacy `hasVariants` flag. Blank size/color values do not count as
  // selectable options.
  const hasSizeOptions = product.variants.some((variant) => Boolean(variant.size?.trim()));
  const hasColorOptions = product.variants.some((variant) => Boolean(variant.color?.trim()));
  ...
  // Determine available sizes and colors. In `both` mode, options remain
  // selectable before the other dimension is chosen, but after one dimension
  // is selected we only expose combinations that actually exist.
  const sizes = Array.from(new Set(product.variants.filter((v) => v.size?.trim()).map((v) => v.size!)));
  const colors = Array.from(new Set(product.variants.filter((v) => v.color?.trim()).map((v) => v.color!)));
  ...
  const selectionReady = variantMode === 'none' || selectedVariant !== undefined;
  const availableStock = variantMode === 'none' ? product.totalStock : (selectedVariant?.stock ?? 0);
  const inStock = selectionReady && availableStock > 0;

  const effectiveQuantity = availableStock > 0 ? Math.min(Math.max(1, quantity), availableStock) : 1;

  const variantActionLabel =
    variantMode === 'size' && !selectedVariantId
      ? t('chooseSize')
      : variantMode === 'color' && !selectedVariantId
        ? t('chooseColor')
        : variantMode === 'both' && !selectedVariantId
  ...
  function addToCart() {
    if (!inStock || (variantMode !== 'none' && !selectedVariantId)) return;
    setAdding(true);
    try {
      addItemToCart({
      ...
      <div className="space-y-3 order-2 lg:order-2">
        ...
        <div className="flex items-center justify-between gap-4 flex-wrap">
          <div className="flex items-center gap-2">
            <div className="flex items-center" aria-label={`${product.avgRating.toFixed(1)} / 5`}>
              {[1, 2, 3, 4, 5].map((star) => (
                <svg key={star} className={`h-4 w-4 ${star <= Math.round(product.avgRating) ? 'fill-amber-400 text-amber-400' : 'text-muted-foreground/30'}`} viewBox="0 0 20 20" aria-hidden="true">
                  <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034a.97.97 0 01-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                </svg>
              ))}
            </div>
            <span className="text-sm text-muted-foreground">
              {product.avgRating > 0 ? product.avgRating.toFixed(1) : '0.0'} · {product.reviewCount} {t('reviewsCount')}
            </span>
          </div>
        ...
        <div className="flex items-center gap-2">
          {!selectionReady ? (
            <span className="text-sm text-muted-foreground font-medium">{variantActionLabel}</span>
          ) : inStock ? (
            <>
            ...
            <div className="flex flex-wrap gap-2">
              {sizes.map((size) => {
                const variant = product.variants.find((v) => {
                  if (v.size !== size || v.stock <= 0) return false;
                  return variantMode !== 'both' || !selectedColor || v.color === selectedColor;
                });
                ...
                return (
                  <button
                    ...
                    onClick={() => {
                      ...
                      if (variantMode === 'size') {
                        const nextVariant = product.variants.find((v) => v.size === size && v.stock > 0);
                        setSelectedVariantId(nextVariant?.id ?? null);
                        return;
                      }
                      if (variantMode === 'both' && selectedColor) {
        ...
        {colors.length > 0 && (
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {colors.map((color) => {
                const variant = product.variants.find((v) => {
                  if (v.color !== color || v.stock <= 0) return false;
                  return variantMode !== 'both' || !selectedSize || v.size === selectedSize;
                });
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      if (variantMode === 'color') {
                        const nextVariant = product.variants.find((v) => v.color === color && v.stock > 0);
                        setSelectedVariantId(nextVariant?.id ?? null);
                        return;
                      }
                      if (variantMode === 'both' && selectedSize) {
                        const nextVariant = product.variants.find(
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 pt-4">
          ...
          <div className="flex items-center border border-border rounded h-10 shrink-0">
            <button
              onClick={() => setQuantity((q) => Math.max(1, Math.min(q, availableStock || 1) - 1))}
          ...
          <button
            onClick={addToCart}
            disabled={!inStock || adding || (variantMode !== 'none' && !selectedVariantId)}
