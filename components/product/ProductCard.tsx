...
export function ProductCard({ product, locale }: { product: ProductCardData; locale: string }) {
  ...
  const variantMode = product.variantMode ?? (product.hasVariants ? 'both' : 'none');
    ...
    <div className="group flex h-full min-w-0 flex-col">
      ...
      <Link href={`/product/${product.slug}`} className="block min-w-0 flex-1">
        <div className="space-y-1 min-w-0">
          ...
          <div className="flex min-w-0 items-center gap-1">
            <div className="flex items-center gap-0.5" aria-label={`${product.avgRating.toFixed(1)} / 5`}>
              {[1, 2, 3, 4, 5].map((star) => (
                <svg
                  key={star}
                  className={`h-3 w-3 ${
      ...
      {inStock ? (
        <button
          ...
          className="mt-auto w-full h-10 sm:h-11 px-3 text-[11px] sm:text-xs font-bold uppercase tracking-wider flex items-center justify-center gap-2 rounded-md transition-opacity hover:opacity-90 shrink-0"
          style={{ backgroundColor: '#1A1A1A', color: '#FFFFFF' }}
        >
          <ShoppingBag className="h-4 w-4 shrink-0" />
          <span className="truncate">{variantActionLabel}</span>
        </button>
      ) : (
        <div className="mt-auto w-full h-10 sm:h-11 px-3 text-[11px] sm:text-xs font-bold uppercase tracking-wider flex items-center justify-center rounded-md bg-muted text-muted-foreground shrink-0">
