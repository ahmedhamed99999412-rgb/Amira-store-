...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({ analysis, products: products.map((p) => {
      const hasSizeOptions = p.variants.some((variant) => Boolean(variant.size?.trim()));
      const hasColorOptions = p.variants.some((variant) => Boolean(variant.color?.trim()));
      const variantMode = hasSizeOptions && hasColorOptions
        ? 'both'
        : hasSizeOptions
          ? 'size'
          : hasColorOptions
