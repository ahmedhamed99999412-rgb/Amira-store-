...
const getFeaturedProductsCached = unstable_cache(async (locale: string, limit: number = 12) => {
  const rows = await db.$queryRaw<Array<{
    ...
    hasVariants: boolean;
    variantMode: 'none' | 'size' | 'color' | 'both';
    variantCount: number;
    name: string;
    ...
    SELECT
      ...
      p."hasVariants",
CASE
        WHEN EXISTS (SELECT 1 FROM "product_variants" pvs WHERE pvs."productId" = p."id" AND NULLIF(BTRIM(pvs."size"), '') IS NOT NULL)
         AND EXISTS (SELECT 1 FROM "product_variants" pvc WHERE pvc."productId" = p."id" AND NULLIF(BTRIM(pvc."color"), '') IS NOT NULL) THEN 'both'
        WHEN EXISTS (SELECT 1 FROM "product_variants" pvs WHERE pvs."productId" = p."id" AND NULLIF(BTRIM(pvs."size"), '') IS NOT NULL) THEN 'size'
        WHEN EXISTS (SELECT 1 FROM "product_variants" pvc WHERE pvc."productId" = p."id" AND NULLIF(BTRIM(pvc."color"), '') IS NOT NULL) THEN 'color'
        ELSE 'none'
      END AS "variantMode",
  ...
  return rows.map((p) => ({
    ...
    hasVariants: p.variantMode !== 'none',
    variantMode: p.variantMode,
    reviewCount: p.reviewCount,
    avgRating: p.avgRating ?? 0,
  }));
}, ['amira-featured-products-v2'], { revalidate: 60 });
...
const getProductsByCategoryCached = unstable_cache(
  async function getProductsByCategory(
    ...
    const orderSql =
      sort === 'price-asc'
        ? Prisma.sql`p."price" ASC, p."createdAt" DESC`
        : sort === 'price-desc'
        ? Prisma.sql`p."price" DESC, p."createdAt" DESC`
        : sort === 'rating'
...
CASE
      ...
      WHERE p."isActive" = true
        ...
        ${onSale ? Prisma.sql`AND p."comparePrice" IS NOT NULL AND p."comparePrice" > p."price"` : Prisma.empty}
      ORDER BY ${orderSql}
      OFFSET ${offset}
      LIMIT ${safePageSize}
    `);
...
}, ['amira-product-by-slug-v2'], { revalidate: 60 });
...
}, ['amira-related-products-v2'], { revalidate: 60 });
