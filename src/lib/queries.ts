...
const QUERY_RETRY_DELAYS_MS = [150, 350, 750] as const;
...
async function safeQuery<T>(fn: () => Promise<T>, fallback: T, context: string): Promise<T> {
  for (let attempt = 0; attempt <= QUERY_RETRY_DELAYS_MS.length; attempt += 1) {
    try {
      ...
      const isLastAttempt = attempt === QUERY_RETRY_DELAYS_MS.length;
      if (isLastAttempt) {
        console.error(`[queries:${context}] query failed after retries; serving fallback data`, error);
        return fallback;
      }

      console.warn(
        `[queries:${context}] transient query failure; retrying (${attempt + 1}/${QUERY_RETRY_DELAYS_MS.length})`,
        error
      );
...
const getStoreSettingsCached = unstable_cache(
  ...
  ['amira-store-settings-v2'],
  { revalidate: 60, tags: ['amira-store-settings'] }
);

export async function getStoreSettings() {
  return safeQuery(() => getStoreSettingsCached(), DEFAULT_STORE_SETTINGS, 'getStoreSettings');
}
...
const getMainCategoriesCached = unstable_cache(
  ...
  ['amira-main-categories-v3'],
  { revalidate: 60, tags: ['amira-main-categories'] }
);

export async function getMainCategories(locale: string) {
  return safeQuery(() => getMainCategoriesCached(locale), [], 'getMainCategories');
}
...
const getHeroBannersCached = unstable_cache(
  async () => db.banner.findMany({
    ...
    select: {
      ...
      ctaTextAr: true, ctaTextEn: true, ctaLink: true, order: true, updatedAt: true,
    },
    orderBy: { order: 'asc' },
  }),
...
export async function getHeroBanners() {
  return safeQuery(() => getHeroBannersCached(), [], 'getHeroBanners');
}
...
export async function getPromoBanners() {
  return safeQuery(() => getPromoBannersCached(), [], 'getPromoBanners');
}
...
}, ['amira-featured-products-v4'], { revalidate: 60, tags: ['amira-products'] });

export async function getFeaturedProducts(locale: string, limit: number = 12) {
  return safeQuery(() => getFeaturedProductsCached(locale, limit), [], 'getFeaturedProducts');
}
...
const getCategoryBySlugCached = unstable_cache(async (slug: string, locale: string) => {
  ...
  }, ['amira-category-by-slug-v1'], { revalidate: 60 });
...
const getProductsByCategoryCached = unstable_cache(
  ...
  { revalidate: 60, tags: ['amira-products'] }
);
...
}, ['amira-product-by-slug-v3'], { revalidate: 60, tags: ['amira-products'] });
...
}, ['amira-related-products-v3'], { revalidate: 60, tags: ['amira-products'] });
