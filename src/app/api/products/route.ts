...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!Number.isSafeInteger(page) || page < 1 || page > 10_000) {
      return apiErrorResponse('INVALID_PAGE', 400, getApiLocale(req.headers.get('x-locale') || req.headers.get('accept-language')));
    }
    if (!Number.isSafeInteger(pageSize) || pageSize < 1 || pageSize > 100) {
      return apiErrorResponse('INVALID_PAGE_SIZE', 400, getApiLocale(req.headers.get('x-locale') || req.headers.get('accept-language')));
    }
    ...
    if ((minPrice !== undefined && (!Number.isFinite(minPrice) || minPrice < 0)) ||
      ...
      return apiErrorResponse('INVALID_PRICE_RANGE', 400, getApiLocale(req.headers.get('x-locale') || req.headers.get('accept-language')));
    }
    ...
    const sort = ['newest', 'price-asc', 'price-desc', 'rating'].includes(rawSort)
      ...
      : null;
    if (!sort) {
      return apiErrorResponse('INVALID_SORT', 400, getApiLocale(req.headers.get('x-locale') || req.headers.get('accept-language')));
    }
