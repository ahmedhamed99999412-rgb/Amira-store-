...
import { apiErrorResponse, internalServerErrorResponse, safeJsonBody } from '@/lib/api-errors';
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (parentId) {
      ...
      // Defensive cycle detection: walk up parent chain to ensure no excessive depth / cycle
      let depth = 0;
      let currentParentId: string | null = parentId;
      const visited = new Set<string>();
      while (currentParentId) {
        if (visited.has(currentParentId)) {
          return NextResponse.json({ error: 'Invalid parent category: circular reference detected' }, { status: 400 });
        }
        visited.add(currentParentId);
        const ancestor = await db.category.findUnique({ where: { id: currentParentId }, select: { parentId: true } });
        currentParentId = ancestor?.parentId ?? null;
        depth++;
        if (depth > 50) {
          return NextResponse.json({ error: 'Parent category chain is too deep' }, { status: 400 });
        }
      }
    }
    ...
    revalidateTag('amira-main-categories', { expire: 0 });
