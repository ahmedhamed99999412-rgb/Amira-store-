...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (shippingCost !== undefined && (existingOrder.status === 'DELIVERED' || existingOrder.status === 'CANCELLED')) {
      return NextResponse.json({
        error: `Cannot modify shipping cost for a terminal order: ${existingOrder.status}`,
        code: 'TERMINAL_ORDER_IMMUTABLE',
      }, { status: 409 });
    }
