...
export async function GET(req: NextRequest) {
  try {
    ...
    const [products, categories] = await Promise.all([
      db.product.findMany({
        ...
        include: {
          ...
          variants: { select: { stock: true, regularPrice: true, salePrice: true, priceAdjustment: true } },
    ...
    const mappedProducts = products.map((p) => {
      ...
      const discount =
        pricing.displayComparePrice !== null && pricing.displayComparePrice > pricing.displayPrice
          ? Math.round(((pricing.displayComparePrice - pricing.displayPrice) / pricing.displayComparePrice) * 100)
          : 0;
      return {
        id: p.id,
        slug: p.slug,
        ...
        comparePrice: pricing.displayComparePrice,
        displayPrice: pricing.displayPrice,
        displayComparePrice: pricing.displayComparePrice,
        priceLabel: formatCurrency(pricing.displayPrice, locale),
        discount,
