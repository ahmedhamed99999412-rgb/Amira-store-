...
async function CategoryProductsSection({
  ...
  const result = await getProductsByCategory(slug, locale, {
    page,
    pageSize: 12,
    minPrice,
    maxPrice,
    ...
    <div className="border-t border-border pt-8">
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-lg font-medium text-brand-charcoal">
          {tCommon('productsCount', { count: result.total })}
        </h2>
      ...
      {result.products.length === 0 ? (
        <div className="text-center py-20">
          <p className="text-muted-foreground">{tCommon('noProductsInCategory')}</p>
        </div>
      ) : (
...
function CategoryProductsFallback() {
  return (
...
export default async function CategoryPage({
  ...
  // Do not block the initial route response on the product grid. The grid is
  // independently cached and can stream behind a stable category shell.
    ...
    <>
      ...
      <main className="flex-1 bg-white">
        <div className="container mx-auto px-4 py-8">
          {/* Breadcrumb */}
          <nav className="flex items-center gap-2 text-xs text-muted-foreground mb-4" aria-label={tCommon('breadcrumb')}>
          ...
          {/* Products section streams independently so the route does not sit on loading.tsx. */}
          <Suspense fallback={<CategoryProductsFallback />}>
