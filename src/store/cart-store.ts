...
function toClientItem(item: ServerCartItem, locale: string): CartItem {
  ...
  const regularBase = variant?.regularPrice ?? item.product.comparePrice ?? item.product.price;
  ...
  persist(
    (set, get) => ({
      ...
      addItem: (item) => {
        ...
        void queueCartRequest(key, async () => {
          ...
          set((state) => {
            const current = state.items.find((entry) =>
              entry.productId === item.productId && entry.variantId === item.variantId
            );
            if (!current) return state;

            const nextQuantity = current.quantity - quantity;
            if (nextQuantity <= 0 && !existingBefore) {
              return { items: state.items.filter((entry) =>
                !(entry.productId === item.productId && entry.variantId === item.variantId)
              ) };
            }
      ...
      updateQuantity: (productId, variantId, requestedQuantity) => {
        ...
        void queueCartRequest(key, async () => {
          ...
          set((state) => ({
            items: state.items.map((item) =>
              item.productId === productId && item.variantId === variantId && item.quantity === quantity
      ...
      getTotalPrice: () => get().items.reduce((sum, item) => addMoney(sum, multiplyMoney(item.price, item.quantity)), 0),
    ...
    }),
    {
      ...
      onRehydrateStorage: () => (state) => {
        if (state) {
          // Notify ServerStateSync after persisted cart hydration completes.
