...
export function Footer({ storeName, locale, whatsappNumber, instagramUrl, facebookUrl }: { storeName: string; locale: string; whatsappNumber: string; instagramUrl?: string | null; facebookUrl?: string | null }) {
  const t = useTranslations('footer');
  const normalizedWhatsapp = formatPhoneForWhatsApp(whatsappNumber);
  ...
  const customerLinks = [
    ...
    ...(normalizedWhatsapp ? [{ label: t('contactUs'), href: `https://wa.me/${normalizedWhatsapp}` }] : []),
  ];
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-8 items-start">
          ...
          <div className="col-span-2 md:col-span-3 lg:col-span-1">
            ...
            <div className="flex gap-2">
              ...
              {instagramUrl && <a href={instagramUrl} target="_blank" rel="noopener noreferrer" aria-label="Instagram" className="w-9 h-9 rounded-full border border-brand-charcoal/20 flex items-center justify-center hover:bg-brand-charcoal hover:text-white transition-colors"><Instagram className="h-4 w-4" /></a>}
              {facebookUrl && <a href={facebookUrl} target="_blank" rel="noopener noreferrer" aria-label="Facebook" className="w-9 h-9 rounded-full border border-brand-charcoal/20 flex items-center justify-center hover:bg-brand-charcoal hover:text-white transition-colors"><Facebook className="h-4 w-4" /></a>}
            </div>
          </div>
