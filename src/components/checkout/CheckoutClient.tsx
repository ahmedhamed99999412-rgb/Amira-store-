...
export function CheckoutClient({ locale }: { locale: string }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSubmitting(true);
    try {
      // Re-read server state immediately before order creation. The order API is
      // still the pricing/stock source of truth, but this prevents checkout from
      // submitting a stale cart after another tab/device changed it.
      ...
      <div className="lg:sticky lg:top-32 h-fit">
        <div className="p-6 border border-border rounded-lg bg-muted/30">
          ...
          <div className="space-y-2 pt-4 border-t border-border">
            ...
            <div className="py-2">
              {couponApplied ? (
                <div className="flex items-center justify-between bg-green-50 border border-green-200 rounded-md px-3 py-2"><span className="text-xs text-green-700 font-medium">{t('couponLabel', { code: couponCode })}</span><div className="flex items-center gap-2"><span className="text-xs font-bold text-green-700">-{formatCurrency(couponDiscount, locale)}</span><button type="button" onClick={() => { setCouponApplied(false); setCouponDiscount(0); setCouponCode(''); }} className="text-xs text-red-600 hover:underline">{t('remove')}</button></div></div>
              ) : (
