...
export function ProductForm({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!form.categoryId || (!regularPrice && !form.differentPriceBySize) || !form.nameAr || !form.nameEn) {
      toast.error(locale === 'ar' ? 'يرجى ملء الحقول المطلوبة' : 'Please fill required fields');
      return;
    }
    if (salePrice && !form.differentPriceBySize && Number(salePrice) >= Number(regularPrice)) {
    ...
    if (form.differentPriceBySize) {
        const invalidSizePrice = variants.some((variant) => {
            if (!variant.size || !variant.size.trim()) return false;
    ...
    try {
      const payload: Record<string, unknown> = {
        ...
        price: form.differentPriceBySize ? null : salePrice || regularPrice,
        comparePrice: form.differentPriceBySize ? null : (salePrice ? regularPrice : null),
        ...
        variants: variants.map((v) => ({
          ...
          stock: v.stock || '0',
          sku: null,
          regularPrice: form.differentPriceBySize && v.size ? (v.regularPrice || null) : null,
          salePrice: form.differentPriceBySize && v.size ? (v.salePrice || null) : null,
        })),
      };
      ...
      <form onSubmit={handleSubmit} className="space-y-6" noValidate>
        ...
        <Card className="p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {!form.differentPriceBySize && (
              <>
                <div className="space-y-2">
                  <Label>{locale === 'ar' ? 'السعر العادي *' : 'Regular Price *'}</Label>
                  <Input
                    type="number"
                    step="0.01"
                    value={regularPrice}
                ...
                <div className="space-y-2">
                  <Label>{locale === 'ar' ? 'سعر البيع (اختياري)' : 'Sale Price (optional)'}</Label>
                  <Input
                    type="number"
                    step="0.01"
                    value={salePrice}
                    ...
                    dir="ltr"
                    min="0"
                    placeholder={locale === 'ar' ? 'اتركه فارغًا بدون خصم' : 'Leave blank for no sale'}
                  />
                </div>
                <div className="space-y-2">
            ...
            {form.differentPriceBySize && (
              <div className="space-y-2">
                ...
                <p className="text-[11px] text-muted-foreground mt-1">
                  {locale === 'ar' ? 'الأسعار بتتحدد لكل متغير من تحت' : 'Prices are set per variant below'}
                </p>
              </div>
            )}
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
              <div className="space-y-2">
                ...
                <Textarea
                  ...
                  dir="rtl"
                  rows={5}
                  placeholder={locale === 'ar' ? 'الوصف الكامل بالعربي' : 'Full description in Arabic'}
                />
              </div>
              <div className="space-y-2">
                ...
                <Textarea
                  ...
                  dir="ltr"
                  rows={5}
                  placeholder={locale === 'ar' ? 'الوصف الكامل بالإنجليزي' : 'Full description in English'}
                />
              </div>
            </div>
          </div>
