          ...
          {items.map((item) => (
            <div key={`${item.productId}-${item.variantId}`} className="flex gap-4 p-4 border border-border rounded-lg">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm font-bold text-brand-charcoal mt-2">{formatCurrency(item.price, locale)}</p>
                <div className="flex items-center justify-between mt-3">
                  <div className="flex items-center border border-border rounded">
                    <button onClick={() => updateQuantity(item.productId, item.variantId, item.quantity - 1)} className="p-2 hover:bg-muted" aria-label={tProduct('decreaseQuantity')}>
                      <Minus className="h-3 w-3" />
                    </button>
                    <span className="px-4 text-sm font-medium" dir="ltr">{item.quantity}</span>
                    <button onClick={() => updateQuantity(item.productId, item.variantId, item.quantity + 1)} className="p-2 hover:bg-muted" aria-label={tProduct('increaseQuantity')}>
              ...
              <div className="text-end">
                <p className="text-sm font-bold text-brand-charcoal">{formatCurrency(multiplyMoney(item.price, item.quantity), locale)}</p>
              </div>
            </div>
          ))}
        </div>
          ...
          <div className="p-6 border border-border rounded-lg bg-muted/30">
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center justify-between text-sm">
                ...
                <span className="font-medium">{formatCurrency(0, locale)}</span>
              </div>
              <div className="border-t border-border pt-3 flex items-center justify-between">
                <span className="font-medium">{t('total')}</span>
                <span className="text-xl font-bold text-brand-charcoal">{formatCurrency(subtotal, locale)}</span>
              </div>
            </div>
