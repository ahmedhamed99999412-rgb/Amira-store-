          ...
          <>
              ...
              {items.map((item) => (
                <div key={`${item.productId}-${item.variantId}`} className="flex gap-3 pb-4 border-b border-border">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm font-bold text-brand-charcoal mt-1">{formatCurrency(item.price, locale)}</p>
                    <div className="flex items-center justify-between mt-2">
                      <div className="flex items-center border border-border rounded text-sm">
                        <button onClick={() => updateQuantity(item.productId, item.variantId, item.quantity - 1)} className="p-1.5 hover:bg-muted" aria-label={tProduct('decreaseQuantity')}>
                          <Minus className="h-3 w-3" />
                        </button>
                        <span className="px-3" dir="ltr">{item.quantity}</span>
                        <button onClick={() => updateQuantity(item.productId, item.variantId, item.quantity + 1)} className="p-1.5 hover:bg-muted" aria-label={tProduct('increaseQuantity')}>
            ...
            <div className="p-4 border-t border-border space-y-3">
              <div className="flex items-center justify-between">
                ...
                <span className="text-lg font-bold text-brand-charcoal">{formatCurrency(total, locale)}</span>
              </div>
